...
router.get("/:id", (req, res) => {
  ...
  if (filmsIndex < 0) return res.sendStatus(404);
...
router.post("/", (req, res) => {
  ...
  const duration =
    typeof req?.body?.duration !== "number" ? undefined : req.body.duration;
  const budget =
    typeof req?.body?.budget !== "number" ? undefined : req.body.budget;
  const link =
  ...
  const findFilm = films.find(
    (film) => film.title.toLocaleLowerCase() === title.toLocaleLowerCase()
  );
...
router.patch("/:id", (req, res) => {
  ...
  if (
    ...
    (duration !== undefined &&
      (typeof req?.body?.duration !== "number" || duration < 0)) ||
    (budget !== undefined &&
      (typeof req?.body?.budget !== "number" || budget < 0))
  )
